...
class ImagePreviewVC: UIViewController, SwipeCardStackDataSource, SwipeCardStackDelegate {
        ...
        if selectedCoverData.premium {
            ...
            if let navigationController = self.presentingViewController as? UINavigationController {
                self.dismiss(animated: false) {
                    if let imageVC = navigationController.viewControllers.first(where: { $0 is ImageVC }) as? ImageVC {
                        ...
                        if let imageVC = storyboard.instantiateViewController(withIdentifier: "ImageVC") as? ImageVC {
                            imageVC.updateSelectedImage(with: selectedCoverData)
