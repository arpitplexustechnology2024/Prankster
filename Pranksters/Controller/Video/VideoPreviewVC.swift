...
class VideoPreviewVC: UIViewController, SwipeCardStackDataSource, SwipeCardStackDelegate {
            ...
            if let navigationController = self.presentingViewController as? UINavigationController {
                self.dismiss(animated: false) {
                    if let videoVC = navigationController.viewControllers.first(where: { $0 is VideoVC }) as? VideoVC {
                        ...
                        if let videoVC = storyboard.instantiateViewController(withIdentifier: "VideoVC") as? VideoVC {
                            videoVC.updateSelectedVideo(with: selectedCoverData)
